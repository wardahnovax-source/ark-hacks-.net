...
const USER_SCREENSHOTS = [
	...
	},
	{
		...
		file: 'ark-wallhack-skeleton.webp',
	},
	{
		source:
...
const REMOVE_PREFIXES = [
	...
	'ark-extract-fight',
	'ark-hacks-aimbot',
	'ark-ascended-hacks-cover',
	'ark-hacks-esp-wallhack',
	'ark-hacks-hero',
	'ark-hacks-logo',
	'ark-ascended-hacks-package',
	'ark-header-art',
	'ark-loadout-builder',
	'ark-player-esp',
	'ark-scav-run-combat',
...
async function generateResponsiveVariants() {
	const heroSource = path.join(imagesDir, 'ark-esp-player-tags.webp');
	const heroMeta = await sharp(heroSource).metadata();

	for (const width of HERO_WIDTHS) {
		if (heroMeta.width && width > heroMeta.width) continue;
		const file = `ark-esp-player-tags-${width}w.webp`;
	...
	for (const { file } of USER_SCREENSHOTS) {
		if (file === 'ark-esp-player-tags.webp') continue;
